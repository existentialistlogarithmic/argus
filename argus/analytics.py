...
Structural measures ask where an entity sits in the network: centrality,
community, distance from known-bad seeds. Domain-agnostic; they work on any
graph.

Typologies ask whether a specific pattern is present: value moving in a closed
loop, payments sized to sit under a reporting threshold, unrelated companies
sharing a registered address. Each encodes something an investigator would look
for, and produces evidence you can show rather than a number you have to trust.

Neither is meaningful before resolution. A cycle through five accounts is
invisible while those accounts are still five hundred unlinked records.
"""
...
@dataclass
class Finding:
    """One generated observation, with the evidence behind it."""
    ...
    # Entities pulled in by following control links, mapped to hop distance.
    # Implication weakens with distance.
...
def betweenness_centrality(graph: KnowledgeGraph, samples: int | None = None,
    ...
    """Brandes, optionally estimated from sampled source nodes.

    Betweenness finds brokers: entities sitting on the only route between
    otherwise separate parts of a network. They are rarely the loudest nodes,
    which is why degree alone misses them.

    Exact computation is O(VE), so beyond a few hundred nodes it is estimated
    from a random sample of sources and rescaled. The ranking stabilizes long
    before the absolute values do.
    """
...
def personalized_pagerank(graph: KnowledgeGraph, seeds: dict[str, float] | None = None,
    ...
    With seeds this answers how close everything is to a set of known-bad
    entities, propagating along relationships in proportion to what each type
    implies about association. Being a company's director means more than
    sharing an employer, and the ontology already says so through each link
    type's weight.
    """
    nodes = list(graph.entities)
    if not nodes:
    ...
    for node in nodes:
        ...
        for neighbour in graph.neighbours(node):
            ...
            # Repeated observations strengthen a relationship, with
            # diminishing returns. 200 payments are not 200x one payment.
...
def _louvain_level(adjacency, strength, total, resolution, rng):
    """One pass of local moving: shift each node to its best community."""
...
def communities(graph: KnowledgeGraph, resolution: float = 1.0, seed: int = 5) -> dict[str, int]:
    ...
    Label propagation was the cheap first choice and it failed badly here: with
    a dense giant component every node adopts its neighbours' label and the
    population collapses into one community with modularity near zero, which is
    worse than useless because it looks like an answer.

    Louvain greedily moves nodes between communities to maximize modularity,
    contracts each community into a node, and repeats. The hierarchical
    contraction is what lets a small tight group survive inside a large loose
    one.
    """
    ...
    for _ in range(10):
        ...
        # Contract: each community becomes one node whose self-loop carries
        # the weight that was internal to it.
        ...
        for node, edges in adjacency.items():
            ...
            for neighbour, weight in edges.items():
                ...
                if neighbour == node:
                    ...
                    # Internal edges are visited from both endpoints; the
                    # self-loop holds the weight once, not twice.
...
def circular_flows(graph: KnowledgeGraph, link_type: str = "transacted",
    ...
    same place. Only repeatedly-used edges count: a single payment completing a
    ring is coincidence, a standing pattern is not.
    """
        ...
        while stack and len(findings) < limit:
            ...
            for other, count, amount in adjacency.get(node, ()):
                if other == start and len(path) >= 3:
                    ...
                    break
                # Only extend through nodes above the start, so each cycle is
                # found once rather than once per member.
...
def structuring(graph: KnowledgeGraph, threshold: float = 10000.0,
                ...
                limit: int = 20) -> list[Finding]:
    """Payments sized to stay under a reporting threshold.

    Legitimate amounts spread across orders of magnitude. A stream of transfers
    clustered just below a round reporting limit and never above it is the
    signature of someone who knows where the limit is.
    """
    ...
    for account, hits in per_account.items():
        ...
        # A few near-threshold payments out of hundreds is noise. The signal
        # is when they dominate the account's outgoing activity.
...
def shared_registration(graph: KnowledgeGraph, min_orgs: int = 3,
                        limit: int = 20) -> list[Finding]:
    """Addresses hosting an implausible number of companies.

    Formation agents legitimately register many firms at one address, so this
    is a lead and not a conclusion. It gets interesting when the companies
    there also share officers, incorporation dates, or counterparties, which
    the correlation step checks.
    """
    findings = []
...
def rapid_incorporation(graph: KnowledgeGraph, window_days: int = 90,
    ...
    The cluster matters, not any single officer. Shell networks are usually
    built by spreading directorships thinly (each person on two boards, nobody
    on five) so that no one name links the whole structure. Companies are
    therefore joined pairwise when they share any officer and were incorporated
    close together, and the connected components of that relation are the
    clusters.
    """
...
def principals(graph: KnowledgeGraph, entity_id: str, depth: int = 2) -> dict[str, int]:
    """Walk control relationships back to the people behind an entity.

    A typology fires on whatever object carries the behaviour, which for
    financial patterns is an account number, and an account number is not a
    suspect. Following ownership and directorship back to people and companies
    turns a pattern into something actionable. It is also what lets findings on
    different object types be recognised as one network.
    """
    found: dict[str, int] = {}
    frontier = {entity_id}
...
def risk_scores(graph: KnowledgeGraph, seeds: list[str], findings: list[Finding],
    ...
    """Blend seeded proximity and typology hits into one score per entity.

    Two independent signals. Proximity says an entity is closely tied to
    something already known bad; typologies say it is itself behaving in a
    recognised way. Combining them stops a well-connected innocent or an
    isolated oddity from reaching the top on its own.
    """
    if not graph.entities:
        return {}
    ...
    # Normalize against a high percentile rather than the max, or a single
    # dominant seed flattens everything else to zero.
    ...
    # Noisy-OR rather than a running sum. Adding severities pins everything
    # the case touches to 1.0 within three findings and erases the ranking.
    # Combining them as independent probabilities keeps later evidence
    # meaningful without saturating.
    for finding in findings:
    ...
    # Seeds are deliberately not pinned to 1.0. That would tie every known
    # subject at the top and destroy the comparison the ranking exists for:
    # which not-yet-known entities look worst.
    return scores
...
def correlate(graph: KnowledgeGraph, findings: list[Finding],
    ...
    """Group findings that touch overlapping sets of entities.

    One typology hit is a lead. Several landing on the same cluster is a case.
    Presenting them separately leaves the analyst to spot the overlap by eye.
    """
    groups: list[dict] = []
    used: set[int] = set()
