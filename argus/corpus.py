...
Clean records make any resolver look good, so this generates hard ones. What
separates real entity resolution from a GROUP BY is what happens when the same
person appears as "Robert J. Whitlock, b. 1974-03-08" in a national registry,
"Bob Whitlock" with only an email in an HR export, and a bare phone number in
comms metadata.

Every source therefore has its own schema, its own subset of the truth, and its
own characteristic corruptions. Ground truth is written separately, never
appears in the source files, and is read only by the evaluator.

A network is planted in the noise: people co-controlling shell companies
registered to one address, moving money in cycles just under a reporting
threshold. None of it shows in a single source; it appears only once resolution
has stitched them together.
"""
...
# --------------------------------------------------------------------------
# corruption helpers, each modelling a specific real-world failure mode
# --------------------------------------------------------------------------
...
class CorpusBuilder:
    ...
    def _make_people(self):
        ...
        for index in range(self.n_people):
            while True:
                ...
                # A few genuine name collisions are left in deliberately, so
                # a resolver that merges on name alone gets punished for it.
    ...
    def _make_background_activity(self):
        ...
        start = date(2024, 1, 1)
        # Scaled to population so a small corpus stays small. A fixed volume
        # over few people makes every selector block enormous and turns a quick
        # test run into a quadratic one.
    ...
    def _plant_network(self):
        ...
        Six principals, five shells at one registered address, cyclic transfers
        structured just below a 10,000 threshold, and dense communication.
        Every strand is split across sources so no single dataset reveals the
        shape.
        """
        rng = self.rng
        ...
        # Each principal sits on at least two shell boards. That overlap is
        # what holds the cluster together under community detection.
        ...
        # Round-tripping: value leaves the first account and comes back, in
        # tranches that never breach the reporting threshold.
        start = date(2024, 3, 4)
        ...
        # Only one principal is known. The rest must be inferred from them.
...
class CorpusWriter:
    ...
    def _write_transactions(self) -> int:
        ...
        # Label the thin account records the payment rail implies, so
        # bank-to-rail merges get scored rather than assumed.
        for account in self.corpus.accounts:
