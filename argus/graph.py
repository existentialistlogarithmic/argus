...
resolved entities. This is where resolution pays off: a transaction between
two bare account numbers becomes an edge between two accounts whose owners
are known.

Parallel observations collapse into a single edge that keeps all of them, so
"these accounts transacted" and "here are the 14 transfers" are one object.
"""
...
class KnowledgeGraph:
    ...
    def build(cls, ontology: Ontology, resolution: ResolutionResult,
        ...
        for link in links:
            ...
            if not src or not dst:
                ...
                continue
            # Resolution can collapse both endpoints into one entity, e.g.
            # two accounts of the same person. The self-loop is real but adds
            # nothing traversable, so it is dropped rather than stored.
            if src == dst:
                continue
    ...
    def find(self, type_name: str | None = None, **predicates) -> list[Entity]:
        """Property-equality search over entities."""
    ...
    def all_paths(self, source: str, target: str, max_hops: int = 4,
        ...
        The shortest connection is rarely what is wanted. Seeing the several
        independent ways two entities are tied together matters more: one path
        is a coincidence, four is a relationship.
        """
