...
Each adapter knows its own schema and nothing else: which columns map to which
ontology properties, and which relationships the source asserts. Adapters make
no identity decisions. That belongs to the resolver, and keeping the two apart
means either can change without the other.

Links are emitted against source-local keys, since no entities exist yet at
ingest time.
"""
...
class Ingestor:
    ...
    def add(self, source: str, local_id: str, entity_type: str, raw: dict) -> str:
        """Emit a record, merging into any existing one with the same key.

        Sources repeat themselves; an employer appears on every staff row. A
        key collision therefore means the same assertion, not a conflict.
        """
        ...
        for row in read_jsonl(path):
            ...
            for officer in row.get("officers", []):
                officer_key = self.add("corpreg", officer["officer_id"], "person", {
                    ...
                    # occupation. It belongs on the officer_of link. Mapping it
                    # here would create false disagreement against the HR job
    ...
    def ingest_transactions(self, path: Path):
        ...
        Every account number seen becomes a record with one property. These
        thin records are the seam. Resolution merges them into the bank's
        fully-described accounts, and only then does a transaction reach a
        person.
        """
        for row in read_jsonl(path):
    ...
    def attach_truth(self, truth_path: str | Path) -> int:
        """Load ground truth for evaluation. The resolver never sees this."""
