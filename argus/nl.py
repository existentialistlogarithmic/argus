...
The LLM never touches the graph. It does exactly one job: turn an analyst's
question into a query in the language defined by ``query.py``, which is then
shown to the analyst and executed deterministically. That boundary is the whole
design. A model that answers from the graph directly can hallucinate an entity,
an edge, or a number, and the analyst has no way to tell. A model that only
writes a query can be wrong -- but wrong *visibly*, in a line of text that can
be read, corrected, and re-run.

The provider is pluggable, and the deterministic fallback is not a stub: it is
a pattern-based translator that handles the common question shapes with no
network access and no API key, so the system is fully usable before anyone
configures a model.
"""
...
class AnthropicProvider(LLMProvider):
    """Claude-backed translation via the official SDK.

    The SDK is imported lazily and only on first use, so the rest of the
    platform keeps its zero-dependency guarantee: the engine runs, and every
    test passes, on a machine that has never installed it.
    """
    ...
    def translate(self, question: str, schema_prompt: str) -> Translation:
        response = self.client().messages.create(
            model=self.model,
            max_tokens=2000,
            # Translation is a short, scoped task -- low effort keeps it fast
            # and cheap without costing accuracy on a grammar this small.
            output_config={
                "effort": "low",
...
class RuleBasedProvider(LLMProvider):
    """Pattern-based translation. No network, no key, fully deterministic.

    It covers the question shapes analysts actually repeat. Anything it cannot
    confidently translate raises rather than guessing -- a wrong query returned
    with confidence is worse than an honest "rephrase that".
    """
...
def schema_prompt(graph: KnowledgeGraph) -> str:
    ...
    property becomes queryable in natural language the moment it is declared --
    no prompt edit required.
    """
    lines = [
        "You translate an intelligence analyst's question into a single query in the",
        "Argus query language. Reply with the query only -- never prose, never an answer.",
        "",
        "GRAMMAR",
        GRAMMAR_HELP,
...
class NaturalLanguage:
    ...
    def translate(self, question: str) -> Translation:
        ...
        # The model's output is never trusted straight into execution -- it is
        # parsed first, and a parse failure is reported rather than run.
