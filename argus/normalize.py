...
Every comparison in the resolver happens between normalized values, never raw
ones. Normalization is deliberately lossy: it strips the variation that carries
no identity signal (case, punctuation, honorifics, formatting) so that the
similarity functions only see variation that actually matters.
"""
...
# "Meridian Holdings" and "Meridian Group" identical, which is exactly the kind
# of over-normalization that manufactures false merges.
_ORG_STOPWORDS = {
...
# Nickname -> canonical given name. Small on purpose: a hand-curated table beats
# a fuzzy match for the handful of names that are genuinely irregular.
NICKNAMES = {
...
def norm_name(value) -> str:
    ...
    Drops honorifics and generational suffixes, expands nicknames, and sorts
    nothing -- token order is preserved because "Smith John" and "John Smith"
    are handled by the token-set similarity, not here.
    """
...
def norm_date(value):
    ...
    Returns a ``date`` or ``None``. Ambiguous day/month orderings are resolved
    by trying ISO first, then day-first, which matches the corpus conventions.
    """
...
def soundex(token: str) -> str:
    ...
    for ch in token[1:]:
        code = codes.get(ch, "")
        # Vowels reset the "previous code" so that doubled sounds separated by a
        # vowel are kept, while genuine repeats collapse.
        if ch in "aeiouy":
            encoded.append("")
            continue
        if ch in "hw":
