"""Pipeline and the investigation surface on top of it.

Pipeline.run takes a directory of raw source files to a scored knowledge
graph. Investigation is the analyst-facing part: dossiers, connection
discovery, and the ranked case list.
"""
...
class Pipeline:
    ...
    def _mark_watchlist(self, graph: KnowledgeGraph) -> list[str]:
        ...
        The payoff of resolution in one line. The watchlist holds only a name
        and a date of birth, but the flag lands on a fully-formed entity with
        accounts, employers and counterparties attached.
        """
        seeds = []
