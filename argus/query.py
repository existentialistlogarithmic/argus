"""A small structured query language over the knowledge graph.

Natural language is ambiguous and an LLM's output is not reproducible; an
analyst's question ultimately has to become something exact, inspectable, and
re-runnable. So the natural-language layer does not query the graph -- it
*compiles to this language*, and the compiled query is shown to the analyst.
What runs is always something they can read, correct, and save.
...
def resolve_field(graph: KnowledgeGraph, entity, name: str, context: dict):
    """Look up a field, including computed ones that aren't stored properties."""
    if name == "risk":
