...
Deciding whether two records describe the same real-world object is the single
hardest thing this platform does, and everything downstream is worthless if it
is wrong. Two failure modes, both fatal:

- *Under-merging* leaves one person as five entities. The network never
  appears, because its edges are spread across fragments.
- *Over-merging* fuses unrelated people into one entity. That invents
  relationships that do not exist, which is far worse -- it manufactures
  evidence against innocent parties.

The approach is Fellegi-Sunter probabilistic linkage: each property
contributes a log-likelihood ratio of evidence, the ratios sum, and the total
is compared against a threshold. A property's weight comes from how often it
agrees for true matches versus how often it collides by chance, so a national
ID outweighs a first name by orders of magnitude without anyone tuning a knob.

Clustering is greedy and conflict-aware rather than plain connected
components, because transitive closure over pairwise matches is how
over-merging happens: A~B and B~C does not make A~C, and a naive union-find
will happily chain a whole population into one blob.
"""
...
# A property discriminating this strongly is treated as an exclusive
# identifier: two different non-empty values are positive evidence of
# *difference*, strong enough to veto a merge outright.
...
class Weights:
    ...
    def evidence(self, name: str, similarity: float) -> float:
        ...
        Full agreement earns the agreement weight and clear disagreement the
        (negative) disagreement weight. In between, the weight is interpolated:
        a near-miss on a date of birth should not be scored as though the two
        records asserted unrelated dates.
        """
...
class Resolver:
    ...
    def score(self, left: Record, right: Record) -> MatchDecision:
        ...
        for name, prop in entity_type.properties.items():
            ...
            # Missing data is not disagreement. A source that never collects a
            # property must not be punished for its silence.
    ...
    def _blocks(self, records: list[Record], entity_type: EntityType, stats: ResolutionStats):
        ...
        for key, members in index.items():
            ...
            if size < 2:
                continue
            # An oversized block means the blocking key lost its selectivity
            # (a default value, an empty string that slipped through). Scoring
            # it is quadratic and almost never productive.
    ...
    def _cluster(self, records: list[Record], decisions: list[tuple[int, int, MatchDecision]],
        ...
        """Greedy conflict-aware agglomeration.

        Pairs are merged strongest-evidence-first. A merge is refused when the
        two clusters assert different values for an exclusive identifier --
        that is what stops one strong name match from chaining together two
        people who demonstrably hold different passports.
        """
...
def evaluate(records_by_type: dict[str, list[Record]], result: ResolutionResult) -> dict:
    """Score resolution against ground truth using pairwise precision/recall.

    Pairwise metrics are the right currency here: they measure decisions the
    system actually made, and they punish a single bad merge in proportion to
    how many spurious pairs it created. Computed from a contingency table
    rather than by enumerating pairs, so it stays linear in records.
    """
    report = {}
    ...
    for type_name, records in sorted(records_by_type.items()):
        ...
        # A cluster is "clean" when every record in it shares one truth id, and
        # "complete" when it holds every record of that truth id.
        exact = 0
...
def find_errors(records_by_type: dict[str, list[Record]], result: ResolutionResult,
                limit: int = 10) -> dict:
    """Surface concrete false merges and misses for inspection.

    An aggregate F1 tells you the system is imperfect; it does not tell you
    *how*. These are the examples an analyst would actually be shown.
    """
    false_merges, missed = [], []
    for type_name, records in sorted(records_by_type.items()):
        labelled = [r for r in records if r.truth_id]
