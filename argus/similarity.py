"""Similarity functions. All return a score in [0, 1].

The only place that decides how alike two values are. resolve.py turns these
scores into match evidence.
"""
...
def jaro_winkler(a: str, b: str, prefix_weight: float = 0.1) -> float:
    ...
    Names agree on their prefix far more often than chance would predict, so
    the boost is signal rather than a tuning knob.
    """
    base = jaro(a, b)
    if base < 0.7:
        return base
...
def _align_tokens(tokens_a: list[str], tokens_b: list[str]) -> tuple[list[float], int]:
    """Pair tokens greedily, best pair first.

    Order matters here. Walking the tokens left to right lets a middle initial
    consume the surname slot, which is why "Robert J Whitlock" failed to match
    "Bob Whitlock". Scoring every candidate pair up front and taking the
    strongest available one each round fixes it.
    """
    if not tokens_a or not tokens_b:
...
def name_similarity(a: str, b: str) -> float:
    ...
    # A token the other name lacks is weak evidence of difference. A missing
    # middle name is far more common than a different person.
...
def org_similarity(a: str, b: str) -> float:
    ...
    Harsher than name_similarity: an unpaired token scores zero instead of
    being discounted. Company names share leading words routinely ("Meridian
    Holdings" and "Meridian Trading" are unrelated firms), so a prefix-boosted
    character comparator is the wrong instrument. What matters is whether every
    token corresponds.
    """
    if not a or not b:
        return 0.0
...
def date_similarity(a, b, tolerance_days: int = 3) -> float:
    ...
    A transposed digit produces a small gap far more often than a genuinely
    different person does, so near-misses keep partial credit.
    """
